...
export default function WeddingEvents() {
  ...
  return (
    <section className="relative overflow-hidden w-full max-w-6xl mx-auto px-6 py-20 bg-gradient-to-b from-[#fcfaf6] to-white">
      {/* Hoa trang trí */}
      <img
      ...
      <div className="grid lg:grid-cols-2 gap-14 items-start">
        ...
        <div className="space-y-8">
          {events.map((event, idx) => (
            <div
              key={idx}
              className="bg-white rounded-[32px] p-8 border border-[#d4af37]/20 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500"
            >
        ...
        <div className="bg-white rounded-[32px] p-8 border border-[#d4af37]/20 shadow-sm">
          ...
          <div className="grid grid-cols-7 gap-3 text-center">
            ...
            {Array.from({ length: 31 }).map((_, i) => {
              const day = i + 1;
              const highlight = [3, 10].includes(day);
